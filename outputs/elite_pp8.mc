...
//   имя — белым (7).
//
//   Клавиатура УКНЦ обслуживается ПП. Пока крутится резидент,
//   штатный getkey() молчит — читаем через pp_getkey() (резидент
//   сам опрашивает клавиатуру). Коды ПП: вправо=91, влево=78.
// ============================================================
...
int main() {
    ...
    while (1) {
        ...
        k = pp_getkey();                                // клавиатура через резидент ПП
        // ПП-скан-коды: вправо = 91 (133 окт), влево = 78 (116 окт).
        // Модели 3 (Python) и 4 (Anaconda) исключены из перелистывания.
        if (k == 91) {                                  // →
            curM = curM + 1;
            if (curM > 6) curM = 0;
            if (curM == 3 || curM == 4) curM = 5;       // перескочить 3,4 вперёд
        }
        if (k == 78) {                                  // ←
            curM = curM - 1;
            if (curM < 0) curM = 6;
            if (curM == 3 || curM == 4) curM = 2;       // перескочить 3,4 назад
        }
        if (curM != shownM) {
            drawName(shownM, 0);
            drawName(curM, 7);
