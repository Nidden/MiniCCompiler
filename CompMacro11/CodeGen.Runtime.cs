...
namespace CompMacro11
{
    ...
    public partial class CodeGen
    {
        private void EmitRuntime()
        {
            ...
            // ── RTFLIP: программный двойной буфер (копированием) ─────
            E("; RTFLIP — flip(): копирование закадрового буфера на экран.");
            E("; В графическом режиме 0 видны ЛЕВЫЕ 40 слов строки (320px).");
            E("; Правые 40 слов (смещение 40..79) не видны — это закадровый");
            E("; буфер. Рисуем туда (x += 320), затем flip() копирует правую");
            E("; половину каждой строки в левую (показываем готовый кадр) и");
            E("; очищает правую для следующего кадра. Аргументов нет.");
            E("RTFLIP:");
            E("        MOV\tR0, -(SP)");
            E("        MOV\tR1, -(SP)");
            E("        MOV\tR2, -(SP)");
            ...
            E("        MOV\tR4, -(SP)");
            E("        MOV\tR5, -(SP)");
            E("        CLR\tR2");                  // R2 = индекс строки*2 (DSPST — слова)
            E("        MOV\t#264., R4");           // 264 строки экрана
            E("RTFLP1: MOV\tDSPST(R2), R3");       // R3 = адрес левого (видимого) слова
            E("        MOV\tR3, R1");
            E("        ADD\t#40., R1");            // R1 = адрес правого (закадрового) слова
            E("        MOV\t#40., R0");            // 40 слов в половине строки
            E("RTFLP2: MOV\tR1, @#176640");        // РА = источник (правая половина)
            E("        MOV\t@#176642, R5");        // R5 = закадровое слово
            E("        MOV\tR3, @#176640");        // РА = приёмник (левая, видимая)
            E("        MOV\tR5, @#176642");        // вывести на экран
            E("        MOV\tR1, @#176640");        // РА = источник снова
            E("        CLR\t@#176642");            // очистить закадровое слово
            E("        INC\tR1");
            E("        INC\tR3");
            E("        DEC\tR0");
            E("        BNE\tRTFLP2");
            E("        ADD\t#2., R2");             // следующая строка (DSPST: +2 байта)
            E("        DEC\tR4");
            E("        BNE\tRTFLP1");
            E("        MOV\t(SP)+, R5");
            E("        MOV\t(SP)+, R4");
            ...
            E("RTLCK1:");
            // Оба x >= 640? (640 = вся ширина строки 80 слов; правые 40 слов —
            // закадровый буфер для flip(). Отсечение по 640, а не 320, как в point.)
            E("        CMP\t4.(R5), #640.");      // x0 >= 640?
            E("        BLT\tRTLCK2");
            E("        CMP\t8.(R5), #640.");      // x1 >= 640?
