...
namespace CompMacro11
{
    ...
    public partial class CodeGen
    {
        private void EmitRuntime()
        {
            ...
            E("        RTS\tPC");
            E("");
            // ── RTPPKEY — pp_getkey(): скан-код, собранный резидентом ПП ──
            // Клавиатура УКНЦ обслуживается ПП. Пока крутится наш резидент,
            // штатный getkey()/.TTINR молчит, поэтому резидент сам опрашивает
            // клавиатуру (PPSCANK) и кладёт скан-код в PPKEY (ОЗУ ЦП). ЦП читает
            // PPKEY напрямую и сбрасывает. 0 = ничего не нажато.
            E("; RTPPKEY — pp_getkey(): вернуть и сбросить скан-код от резидента ПП.");
            E("RTPPKEY:");
            E("        MOV\tPPKEY, R0");
            E("        CLR\tPPKEY");
            E("        RTS\tPC");
            ...
            E("PPRES:");
            E("PPRLP:  JSR\tPC, PPSCANK");          // опросить клавиатуру → PPKEY
            ...
            E("        RTS\tPC");
            E("");
            // PPSCANK — опрос клавиатуры ПП. Если код готов — пишет его в PPKEY
            // (ОЗУ ЦП, косвенно через 177010). 177700 = состояние (бит7=готов),
            // 177702 = скан-код. Портит R0, сохраняет остальное.
            E("PPSCANK: TSTB\t@#177700");           // клавиша готова?
            E("        BPL\tPPSKX");
            E("        MOV\t@#177702, R0");          // скан-код
            E("        BIC\t#177600, R0");           // 7 бит (убрать parity/флаги)
            E("        BEQ\tPPSKX");                 // 0 → ничего не писать
            E("        MOV\t#<PPKEY/2>, @#177010");  // адрес PPKEY в ОЗУ ЦП
            E("        MOV\tR0, @#177014");          // записать скан-код
            E("PPSKX:  RTS\tPC");
            E("PPREND:");
            E("");
            ...
            E("PPKEY:  .WORD\t0");                // скан-код клавиши (ПП→ЦП)
