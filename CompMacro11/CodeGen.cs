...
namespace CompMacro11
{
    ...
    public class CodeGen
    {
        ...
        public string Generate(ProgramNode prog)
        {
            ...
            E("");
            E("        .END\tMAIN");
            return Peephole(_out.ToString());
        }

        // ── Peephole оптимизатор ─────────────────────────────────
        // Работает на уровне строк сгенерированного Macro-11.
        // Применяет локальные замены пар/троек инструкций.
        private static string Peephole(string src)
        {
            var lines = src.Split('\n');
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                string prev = result.Count > 0 ? result[result.Count - 1] : "";
                ...
                // ── Паттерн 2: MOV X, R0 / TST R0 → убрать TST ──
                ...
                // ── Паттерн 3: CLR R0 / TST R0 → убрать TST ─────
                ...
                // Если только что записали X из R0, загружать X назад не нужно
                ...
                // ── Паттерн 5: MOV R0, R1 / MOV R1, R0 (уже в п.1)
                // MOV Rx, R0 / MOV R0, Rx → MOV Rx, R0 (если Rx не R0)
                ...
                // Если источник — память (не регистр), MOV не нужен:
                // TST ставит те же флаги N и Z без загрузки в регистр.
                // Примеры: MOV -4.(R5),R0 / TST R0  →  TST -4.(R5)
                //          MOV 4.(SP),R0  / TST R0  →  TST 4.(SP)
                // НЕ применять если источник — регистр (TST reg уже должен быть без MOV)
                if (IsMov(curT, out string c6f, out string c6t) &&
                    c6t == "R0" && !IsRegister(c6f) &&
                    nextT == "TST\tR0")
                {
                    // Заменить пару на одну инструкцию TST mem
                    result.Add("        TST\t" + c6f);
                    i++; // пропустить TST R0
                    continue;
                }

                // ── Паттерн 8: JMP label / label: → убрать JMP ──────
                // ОТКЛЮЧЁН — может удалить нужные метки для switch/do-while
                // if (curT.StartsWith("JMP\t")) { ... }

                // ── Паттерн 11: цепочки JMP→JMP — ОТКЛЮЧЁН ──────────
                // if (curT.StartsWith("JMP\t")) { ... }
                // ADD коммутативна — результат сразу в R0, MOV не нужен
                ...
                // ── Паттерн 10: CLR R0 / ADD X,R0 → MOV X,R0 ───────
                // CLR R0 + ADD mem = MOV mem (если mem ≠ регистр)
                if (curT == "CLR\tR0" && nextT.StartsWith("ADD\t"))
                {
                    string addArgs = nextT.Substring(4).Trim();
                    int comma10 = addArgs.IndexOf(',');
                    if (comma10 > 0)
                    {
                        string src10 = addArgs.Substring(0, comma10).Trim();
                        string dst10 = addArgs.Substring(comma10 + 1).Trim();
                ...
                // ── Паттерн 11: цепочки JMP→JMP — ОТКЛЮЧЁН ─────────
                // Может перенаправить JMP минуя определение метки → undefined
